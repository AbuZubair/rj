@extends('layouts.admin', ['activePage' => 'purchase_order', 'titlePage' => __('Purchase Order')])
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-lg-12" style="overflow-x:auto !important">
        ...
        <table id="dynamic-table" class="table yajra-datatable display nowrap" delete-url="{{url('item-transaction/purchase-order/delete')}}" edit-url="{{url('item-transaction/purchase-order/edit')}}" data-modal="poModal" data-checkbox="pos">
            <thead>
                <tr>
...
<div class="modal fade modal-transaction modal-full" id="poModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="padding-right:0 !important">
  <div class="modal-dialog">      
    <div class="modal-content">
      <div class="modal-header pl-4 bg-info text-white">
        <h5 class="modal-title" id="poModalTitle">Add</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        @include('item_transaction.po.form')
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
        ...
        columns: [
            {data:null,render:function(data,type,full,meta){
              let disabled = (data.status>0)?'disabled':''
                return '<input class="ml-md-0 ml-4" type="checkbox" name="pos" '+disabled+' value="'+data.id+'" onClick="singleToggle()"/>';                    
              },
  ...
  function getDropdown(){
    return new Promise((resolve,reject) => {
      $.ajax({
        url : '{{url("item-transaction/purchase-order/dropdown-params")}}',
        type: 'GET',
        data: '',
  ...
  function generatePONo() {
    const d = new Date();
    $('input[name=transaction_no]').val(`PO${d.getFullYear()+("0" + (d.getMonth() + 1)).slice(-2)}${String(Date.now()).slice(-4)}`)
  }
