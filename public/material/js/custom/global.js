...
function generateBreadcrumbs() {
  ...
  for( var i = 0; i < here.length; i++ ) {
      ...
      html += `<span>${capitalizeFirstLetter(text.replace(/-/g," "))}</span>`
  } 
...
function toggle(source) {
  ...
  for(var i=0, n=checkboxes.length;i<n;i++) {
    if(!$(checkboxes[i]).prop('disabled'))checkboxes[i].checked = source.checked;
  }
}
          ...
          input.each(function(){
            ...
            if($(that).attr('id') && !$(that).attr('id').includes('btn-vedit')){
              ...
              if($(that).attr('data-type')){
                val = formatCurrency(jsonResponse.data[$(that).attr('id')])
              }else{
                if(($(that).attr('id') == 'transDate') && !jsonResponse.data[$(that).attr('id')]){
                  val = jsonResponse.data.trans_year+'-'+jsonResponse.data.trans_month+'-'+jsonResponse.data.trans_date;
                }else{
              ...
              $(that).val(val)
              if ($(that).is(':radio')){
                $(that).val(jsonResponse.data[$(that).attr('name')])
                if(jsonResponse.data[$(that).attr('name')].toString() === $(that).attr('data-value')){
              ...
              if ($(that).hasClass("select2-hidden-accessible")) {
                let _val = jsonResponse.data[$(that).attr('id')]
                let val = (_val && _val.toString().includes(',')) ? _val.split(',') : _val
...
function addItem() {
  ...
  let subtotal = 0, qty = 0, harga = 0, disc = 0;
    ...
    input_el.children().each(function (index) {
      ...
      if(el.is('input') || el.is('select')){
        ...
        if(el_child.is('input')){
          ...
          el_child.attr('name', `${name}[${idx}]`).attr('id', id+idx).prop('readonly',true).val(el.val())
          if(name.includes('qty'))qty = parseInt(el.val().replaceAll('.', ''));
          if(name.includes('harga'))harga = parseInt(el.val().replaceAll('.', ''));
          if(qty != 0 && harga != 0){
          ...
          if(name=='disc'){
            if(el.val()){
              ...
              el_child.val(disc.toString().replace(".00","").replace(/\B(?=(\d{3})+(?!\d))/g, "."))
            }
            discEl = el_child
          }
          if(name.includes('discamt')){
            if(el.val()!= ''){
              if(disc!=0)subtotal = (subtotal + disc) - el.val().replaceAll('.', '')
                else subtotal = subtotal - el.val().replaceAll('.', '');
              discEl.val(el.val());
            }
          }
        }
        ...
        el_child.attr('name', `${name}[${idx}]`).attr('id', id+idx).prop('readonly',true).val(input_val.val())
        selectedItem = input_val.val()
      }else if(el.children().find('i')){
        el_child.removeAttr('onclick')
        el_child.children().last().removeAttr('onclick');
        el_child.children().last().attr('onClick', 'removeItem('+idx+')');
      }
      
      if(el.hasClass('form-check')){
        const val = el.children().children().first().val()
        let html = `<td class="col-md-1"><input type="hidden" name="is_master_update[${idx}]" value="${val}">${val==1?'<span class="badge badge-success">update</span>':''}</td>`
        cloned.append(html)
      }

      if($(this).hasClass('sub-total')){
        total +=  subtotal
        child.text(subtotal.toString().replace(".00","").replace(/\B(?=(\d{3})+(?!\d))/g, "."))
      }
  ...
  wait.then((res) => {
    ...
    renderTfoot();
    input_el.children().find('input[type=checkbox]').prop('checked',false).trigger('change')
...
function removeItem(idx) {
  let subtotal = item_table.find(`tr[data-row=${idx}]`).find('.sub-total').text();
  items.splice(idx, 1)
  ...
  item_table.children().each(function () {
    if(parseInt($(this).attr('data-row')) > parseInt(idx)){
      ...
      $(this).children().each(function (idx) {
        let that = this;
        if(idx==0)$(that).empty().text((index-1)+1)
        let el = $(that).children().last()
        if(el.is('input')){
          let name = el.attr('name').replace(`[${index}]`,`[${index-1}]`)
          let id = el.attr('id').replace(`-${index}`,`-${index-1}`)
          el.attr('name', name).attr('id', id)
        }else{
          $(that).children().last().removeAttr("onclick"); 
          $(that).children().last().off("click");
          $(that).children().last().on("click", function () { 
            removeItem(index-1)
          });
        }
      });
...
function renderTfoot(){
  if(total>0){
    $('tfoot').show()
    $('.total-items').text(total.toString().replace(".00","").replace(/\B(?=(\d{3})+(?!\d))/g, "."))
  }else{
    $('tfoot').hide()
  }
...
function editItemTransaction(id,req = null,type=null) {
  ...
  if(req=='view'){
    ...
    input_el.children().find('.add-btn').removeAttr('onclick').css("opacity",0.5)
    setTimeout(() => {
    ...
    if(!input_el.children().find('.add-btn').attr("onclick"))input_el.children().find('.add-btn').attr('onclick','addItem()').css("opacity",1)
          ...
          for (let index = 0; index < rows.length; index++) {
            ...
            cloned.children().each(function (idx) {
              ...
              if(el.is('input')){
                ...
                let name = el.attr('name').replace(`-sample`,``)
                let val;
                if(name.includes('item'))val = `${rows[index].item_name} (code:${rows[index].item_code})`
                else if(name.includes('qty'))val = rows[index].quantity
                else if(name.includes('harga'))val = rows[index].harga.toString().replace(".00","").replace(/\B(?=(\d{3})+(?!\d))/g, ".")
                else if(name.includes('satuan'))val = rows[index].satuan
                else if(name.includes('konversi'))val = (rows[index].konversi)?rows[index].konversi:rows[index].konversi_master;
                el.attr('name', `${name}[${index}]`).attr('id', id+index).prop('readonly',true).val(val)
              }
              ...
              if(el.children().find('i')){
                ...
                else btn.attr('onClick', 'removeItem('+index+')');
              }
  
              if($(this).hasClass('sub-total')){
                $(this).text(subtotal.toString().replace(".00","").replace(/\B(?=(\d{3})+(?!\d))/g, "."))
              }
            })
            cloned.attr('data-row',index).removeClass('d-none').addClass('d-flex');
            item_table.append(cloned);
            renderTfoot();
          }
          resolve()
...
function formatCurrency(data){
  data = (data)?data:0;
  return data.toString().replace(".00","").replace(".", ',').replace(/\B(?=(\d{3})+(?!\d))/g, ".")
}
